...
void main() {
  ...
  group('Search Gifs', () {
      blocTest<GiphyCubit, GiphyState>(
        ...
        build: () {
          when(mockGifRepository.searchGif(
              apikey: anyNamed('apikey'), offset: anyNamed('offset'), keyword: anyNamed('keyword'),language: anyNamed('language')))
              .thenAnswer((_) async => Right(GiphyGif(
              data: [],
              meta: Meta(status: 200, msg: 'OK', responseId: 'test_id'))));
          return GiphyCubit(gifRepository: mockGifRepository);
        ...
        act: (cubit) => cubit.searchGif(
            apikey: 'test_api_key', offset: 0, keyword: 'test_keyword', isFirstFetch: true,language: GiphyLanguage.English),
        expect: () => <GiphyState>[
          GiphyLoading(),
          SearchGifSuccess(
              gif: GiphyGif(
      ...
      blocTest<GiphyCubit, GiphyState>(
        ...
        build: () {
          when(mockGifRepository.searchGif(
              apikey: anyNamed('apikey'), offset: anyNamed('offset'), keyword: anyNamed('keyword'),language: GiphyLanguage.English))
              .thenAnswer((_) async => Right(GiphyGif(
              data: [],
              meta: Meta(status: 200, msg: 'OK', responseId: 'test_id'))));
          return GiphyCubit(gifRepository: mockGifRepository);
        ...
        act: (cubit) => cubit.searchGif(
            apikey: 'test_api_key', offset: 0, keyword: 'test_keyword', isFirstFetch: false,language: GiphyLanguage.English),
        expect: () => <GiphyState>[
          SearchGifSuccess(
              gif: GiphyGif(
                  data: [],
